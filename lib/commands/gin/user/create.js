...
class Create extends command_1.SfdxCommand {
    async run() {
        ...
        if (this.flags.rolename != null) {
            const result = await conn.query(`SELECT Id, Name FROM UserRole WHERE Name = '${this.flags.rolename}'`);
...
Create.flagsConfig = {
    generateuniqusername: command_1.flags.boolean({ char: 'a', default: true, description: 'Generate a unique user name' }),
    ...
    rolename: command_1.flags.string({ char: 'r', description: 'Role name' })
};
Create.requiresUsername = true;
Create.supportsDevhubUsername = false;
