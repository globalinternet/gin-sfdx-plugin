...
class GenerateCustomMetadataFile extends command_1.SfdxCommand {
    async run() {
...
`.trim()
    ...
    async generateConfigFile() {
        ...
        const data = await core_1.fs.readFile(fileName)
            .then(data => JSON.parse(data.toString('utf-8')));
        const files = [];
        const projectConfig = await this.project.resolveProjectConfig();
        const packageFolder = projectConfig.packageDirectories.find(_ => _.default);
        ...
        await Promise.all(files.map(file => core_1.fs.writeFile(file.name, file.body)));
        this.ux.stopSpinner(`successfully created ${files.length} files`);
    }
}
exports.default = GenerateCustomMetadataFile;
GenerateCustomMetadataFile.description = 'Waits until all Permission Set Groups are updated';
GenerateCustomMetadataFile.examples = [
    `$ sfdx gin:bourne:generatecustommetadata
    custom metadata generated
    `
];
GenerateCustomMetadataFile.flagsConfig = {};
GenerateCustomMetadataFile.requiresUsername = false;
GenerateCustomMetadataFile.requiresDevhubUsername = false;
GenerateCustomMetadataFile.requiresProject = true;
//# sourceMappingURL=generateCustomMetadata.js.map